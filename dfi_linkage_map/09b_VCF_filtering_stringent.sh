...
module load vcftools/0.1.16 bcftools/1.23.1 bedtools/2.31.1 tabix/0.2.6 GATK/4.5.0.0
...
zcat $FILTVCF/${DATASET}_gatk_filtered_stringent.vcf.gz | awk -F "\t" -v OFS="\t" '{
  if ($0 ~ /^#/) {
    print
  } else if ($7 == "PASS") {
    print
...
}' > $FILTVCF/${DATASET}_gatk_filtered_stringent_pass.vcf && bgzip $FILTVCF/${DATASET}_gatk_filtered_stringent_pass.vcf

tabix -p vcf $FILTVCF/${DATASET}_gatk_filtered_stringent_pass.vcf.gz
...
tabix -p vcf $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.vcf.gz

#FILTER SNPS AROUND INDELS
#isolate SNPs
vcftools --gzvcf $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.vcf.gz \
--remove-indels --maf 0.0000001 \
--recode --recode-INFO-all \
--out $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.snp
bgzip $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.snp.recode.vcf

#isolate indels
vcftools --gzvcf $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.vcf.gz \
--keep-only-indels --maf 0.0000001 \
--recode --recode-INFO-all \
--out $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.indel
bgzip $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.indel.recode.vcf

#get the header
bcftools view -h $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.vcf.gz > $FILTVCF/header.txt

#print records that don't overlap with indels or 5bp in either direction
bedtools window -v -a $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.snp.recode.vcf.gz \
-b $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic.indel.recode.vcf.gz \
-w 5 > $FILTVCF/variant.rm_indel_mark.vcf

#put filtered records and header together
cat $FILTVCF/header.txt $FILTVCF/variant.rm_indel_mark.vcf > $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic_indels.vcf
bgzip $FILTVCF/${DATASET}_gatk_filtered_stringent_pass_biallelic_indels.vcf
