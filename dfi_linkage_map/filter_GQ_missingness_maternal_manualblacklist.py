    ...
    for line in f:
        ...
        if line.startswith("#") == True:
            if line.startswith("#CHROM") == True:
                ...
                for i in [2,27,31,32,41,42,45,48,63,66,70,74]:
                    if i not in blacklist_indices:
                        blacklist_indices.append(i)
                    else:
                        continue
                blacklist_indices.sort()
                samples_filt=[sample for i,sample in enumerate(samples) if i not in blacklist_indices]
            ...
            #if mat geno is above threshold
            if mat_gq < gq_threshold:
                mat_poor_count+=1
                continue
            else:
