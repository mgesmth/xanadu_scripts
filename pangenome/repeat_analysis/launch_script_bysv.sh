...
cd ${log}
sbatch --array=[0-${array_num}] ${pgscripts}/filtermatches_bysv.sh ${threshold} | cut -d ' ' -f4 > rmjid.tmp
rmjid=$(tr -d '\n' < rmjid.tmp)

#Merge surviving matches
sbatch -d afterok:${rmjid} ${pgscripts}/merge_surviving_alignments_bysv.sh ${threshold}
