...
with open(input_outfile, "r") as f, open(output_outfile, "w") as of, open(error_file, "w") as ef:
    header=["raw_score","e-value","sv_name","sv_start","sv_end","sv_len","sv_prop","gene_name","gene_start","gene_end","gene_len","gene_prop"]
    of.write('\t'.join(header) + '\n')
    ef.write('\t'.join(header) + '\n')
    for line in f:
        fields=line.strip().split()
        #skip alignments when theres a better scoring match overlapping it
        sv_start=int(fields[3].strip("()"))
        sv_end=int(fields[4])
        sv_len=int(fields[5])
        ...
        gene_start=int(fields[8].strip("()"))
        gene_end=int(fields[9])
        gene_len=int(fields[10])
        if fields[6] == "plus":
